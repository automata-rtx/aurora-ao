...
| R12 | **A capture is one mesh and one synthesised skeleton per draw call.** `GameCapturer` keys meshes on the per-draw geometry hash, and `game_exporter.cpp` writes one `UsdSkelSkeleton` per mesh whose joints `generateSkeleton` *invents* — named `root`, `root/joint1`… in a flat list, with translation-only bind transforms placed at the weighted centroid of each bone's vertices | A character arrives in the capture as dozens of separate hashes, each with its own unrelated skeleton, and on the matrix-palette path each chunk has a **single** bone, because R5's compaction hands Remix only the matrices that packet referenced. Pulling a whole body out of a capture, or authoring one replacement for it, is impractical | **Open, diagnosed 2026-08-08, not built.** The deformation is *correct* — the invented bind pose cancels out of UsdSkel's `inverse(bindTransform) · jointSkelXform`, derived by reading both functions — so only the skeleton's *appearance* and the fragmentation are wrong. Neither can be fixed in the fork alone: merging needs a global joint index space (which the matrix-palette path does not have, since a GX position-matrix slot is reused across packets) and a real skeleton needs `J3DJointTree`'s names, parents and bind matrices. Three-stage design in `dusklight-ao/docs/remix-open-issues.md` issue 15. Measurable today from the fork's `capture.geometry` log line: a bone histogram piled up at `1:` is this path, large equal counts are the envelope path |
