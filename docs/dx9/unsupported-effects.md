...
| R11 | **No lerp between two constants** in stock Remix. The one stage it reads offers `TEXTURE`, `DIFFUSE`, `TFACTOR`, `CURRENT` and ops reducing to `t·C` or `t + C` | This game's dominant material shape is `lerp(colourA, colourB, texture)` (§7), so every two-colour ramp was an approximation — Goron Mines lava reproduced as red-to-white | **Fixed in the fork 2026-08-04, untested.** Both endpoints are carried to `RtSurface` and the shader evaluates the GX combiner directly. No struct growth: the spare bits were already there. [`remix-material-interface.md`](remix-material-interface.md) §10 |
