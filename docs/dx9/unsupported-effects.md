...
| R5 | **`MaxVertexBlendMatrixIndex` is not enforced** — Remix reads the transform state directly and skins on the GPU | Masked a real raw-D3D9 bug for weeks: GX's 10-deep matrix palette overran the device's 9-index cap and scattered vertices, while Remix looked perfect | Palette is compacted per draw, and overflow draws are split into per-palette groups — **except for draws carrying a model identity (2026-08-08)**, which deliberately spend this non-enforcement: they write the model's *global joint index* as the blend index and load the whole joint palette, so every draw of a character agrees on what bone 7 means. That is what makes one replacement able to stand in for a whole body (R12). The raw D3D9 image scatters those models, which §0 says is not a cost |
...
| R12 | **A capture is one mesh and one synthesised skeleton per draw call.** `GameCapturer` keys meshes on the per-draw geometry hash, and `game_exporter.cpp` writes one `UsdSkelSkeleton` per mesh whose joints `generateSkeleton` *invents* — named `root`, `root/joint1`… in a flat list, with translation-only bind transforms placed at the weighted centroid of each bone's vertices | A character arrived in the capture as dozens of separate hashes, each with its own unrelated skeleton, and on the matrix-palette path each chunk had a **single** bone, because R5's compaction handed Remix only the matrices that packet referenced. Pulling a whole body out of a capture, or authoring one replacement for it, was impractical | **Built across all three repos 2026-08-08/09 — CI-green, NOT tested in game.** The game publishes its joint tree and slot mapping, this backend composes that with its palette compaction and switches those draws to global joint numbering (§6(a′) of [`gx-to-d3d9-mapping.md`](gx-to-d3d9-mapping.md)), and the fork merges a character's draws into one captured mesh carrying the real armature, then resolves one replacement for it at runtime. The deformation was *correct* all along — the invented bind pose cancels out of UsdSkel's `inverse(bindTransform) · jointSkelXform`, derived by reading both functions — so what changed is the fragmentation and the skeleton's *appearance*. **Two caveats, both live:** the first build crashed on launch (null `mInvJointMtx` on any model without an EVP1 envelope block; fixed 2026-08-09, also untested), and a merged character's vertices are still in **two spaces** — rigid packets are joint-local, envelope packets are model-space — so the posed body is right but the *rest* pose is not. Both, plus the worked-out fix for the second, are in `dusklight-ao/docs/remix-open-issues.md` issue 15. Measurable from the fork's `capture.geometry` and `capture.merge` lines |
