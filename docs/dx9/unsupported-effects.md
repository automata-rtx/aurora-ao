...
| R10 | **Emissive colour is a constant *or* a texture, never both**, and whichever it is then gets run through the **albedo's** texture op with the emissive substituted for the texture sample | A glow set to a colour arrives as `op(colour, tFactor)`. Worse, a *constant* glow at any useful intensity swamps the albedo: the Goron Mines lava read as one flat hot colour with no crust (tested 2026-08-04) | **Sidestepped rather than worked around.** The fork takes the reconstructed albedo as the glow — a self-lit surface glows the colour it appears — so no separate emissive colour is built and there is nothing for the op to corrupt. See [`remix-material-interface.md`](remix-material-interface.md) §9 |
